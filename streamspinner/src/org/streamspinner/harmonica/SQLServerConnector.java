...
/**
 * SQL Server 2005 (or 2000) と接続するためのクラス．<BR>
 * <BR>
 * 変更履歴：
 * <PRE>
 * </PRE>
 * @author snic
 * @version 1.0 (2006.8.21)
 */
public class SQLServerConnector implements DBConnector {
	...
	throws HarmonicaException{
		...
		String jdbc_url = "jdbc:sqlserver://"+url+
			";database="+db+
			";user="+user+
			";password="+password;
		debug(jdbc_url);
		...
		try{
			...
			throw new HarmonicaException("can't connect SQL Server.",e);
		}
	}
	...
	public List<Schema> getSchemaList() throws HarmonicaException{
		try{
			...
			while(rs.next()){
				...
				if(table_name.equals("sysdiagrams")) continue;
				...
				while(rsc.next()){
					...
					column_types.add(convertType(rsc.getInt(5)));
				}
				rsc.close();
	...
	public void disconnect() throws HarmonicaException{
		try{
			...
			debug("Disconnect to SQL Server!");
		}catch(Exception e){
	 ...
	 */
	 public String convertDataType(String ss_type){
		if(ss_type.equals(DataTypes.STRING)) return "TEXT";
		if(ss_type.equals(DataTypes.LONG)) return "BIGINT";
		if(ss_type.equals(DataTypes.DOUBLE)) return "FLOAT";
		return "BLOB";
	}
	...
	private void debug(Object o){
		HarmonicaManager.debug("SQLServer",o);
	}
}
