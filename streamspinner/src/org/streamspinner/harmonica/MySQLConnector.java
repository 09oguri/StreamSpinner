 ...
 * 変更履歴：
 * <PRE>
 * 1.2 2006.8.4 書込レートの計算に索引も考慮．
 * 1.1 2006.8.3 仕事人の変更
 * </PRE>
 * @author snic
 * @version 1.2 (2006.8.4)
 */
public class MySQLConnector implements DBConnector {
	...
	throws HarmonicaException{
		...
		String jdbc_url = "jdbc:mysql://"+url+"/"+db+option;
		debug(jdbc_url);
		...
		try{
			...
			throw new HarmonicaException("can't connect MySQL.");
		}
	}
	...
	updateInsertionRate(Schema[] schemas, long time){
		...
		for(int i=0;i<schemas.length;i++){
			String[] names = {"harmonica_rate_estimate.timestamp"};
	...
	public Schema getSchema(String table_name) throws HarmonicaException{
		try{
			...
			while(rs.next()){
				...
				while(rsc.next()){
					column_names.add(table_name+"."+rsc.getString(4));
					column_types.add(DataTypes.convertSQLType(rsc.getInt(5)));
				}
				rsc.close();
				String[] attr_names = column_names.toArray(new String[0]);
				String[] attr_types = column_types.toArray(new String[0]);
	...
	/**
	 * StreamSpinner用のデータ型をMySQL用のデータ型に変換する．
	 *
	...
	private void debug(Object o){
		HarmonicaManager.debug("MySQL",o);
	}
}
